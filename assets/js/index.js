...
const getURL = "https://betel-couples-dinner-server.onrender.com/api/couples/"
const postURL = "https://betel-couples-dinner-server.onrender.com/api/addcouple/"
const deleteURL = "https://betel-couples-dinner-server.onrender.com/api/deletecouple/"
...
function addToTable(doc) {
  ...
  cell1.innerHTML = doc.couple
  cell2.innerHTML = doc.paid
  cell3.innerHTML = "<input type='radio' couple='select' onclick='selectRow(this)' checked>"
...
function setSelection(row) {
  ...
  document.getElementById("couple").value = row.cells.item(0).innerHTML
  document.getElementById("paid").value = row.cells.item(1).innerHTML
...
function addData() {
  ...
  if (!couple) {
    alert("couple is required!!")
    document.getElementById("couple").focus()
    return
  }
  if (!paid) {
  ...
  postToDB({ couple: couple, paid: paid })
}
  ...
  fetch(postURL, {
    ...
    .then(data => {
      ...
      if (id) {
        doc._id = id
        addToTable(doc)
        document.getElementById("status").innerHTML = "Adicionado com sucesso!"
...
function deleteFromDB(id) {
  ...
  fetch(deleteURL + id, { method: "DELETE" })
    ...
    .then(data => {
      document.getElementById("status").innerHTML = "Removido com sucesso!"
...
function selectTopOrBottomRow(n) {
  ...
  if (table.rows.length < 2) {
    document.getElementById("status").innerHTML = "Sem dados."
    return
  }
  ...
  setSelection(row)
  document.getElementById("status").innerHTML = "Selecionado linha " + selectedRowIx
